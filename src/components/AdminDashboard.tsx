...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  const testConnection = async () => {
    ...
    try {
      // Test 1: Direct REST API call with proper headers
      ...
      setDebugInfo(`Direct API connection successful!`);
      
      // Test 2: Basic connection with simple query via Supabase client
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const projectsUrl = `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/projects?select=user_id`;
